...
| Arquivo | O que é |
|---|---|
| `AlfaMatriz Sistema.dc.html` | **O design final.** As 13 telas + login, tema claro/escuro, sidebar expansível, navegável pela sidebar. É a referência principal. |
...
10. **Quadro do funil restrito verticalmente.** Precisa de `height:calc(100vh - 120px)` na tela,
    `flex:1; min-height:0` no quadro e `align-items:stretch` nas colunas, com a lista de cards rolando por dentro.
