#!/usr/bin/env bash
# executar-tarefas.sh — gerado por `onp-spec plano permissoes-editar-e-imprimir` em 2026-08-14 22:26
...
#
# resumo do que está rolando, a qualquer momento: onp-spec resumo permissoes-editar-e-imprimir
set -u
set -o pipefail

RUN_ID='AlfaMatriz-permissoes-editar-e-imprimir-mstinh81'
FEATURE='permissoes-editar-e-imprimir'
BASE_BRANCH='spec/permissoes-editar-e-imprimir'
...
preparar_ambiente() {
  ...
  git ls-files --error-unmatch -- '.spec/features/permissoes-editar-e-imprimir/spec.md' >/dev/null 2>&1 || falhar "spec.md não está commitada — os worktrees das faixas precisam dela no git"
  ...
  git worktree prune
  LOG_DIR="$(dirname "$TOPLEVEL")/onp-worktrees/AlfaMatriz-permissoes-editar-e-imprimir-logs"
  WT_BASE="$(dirname "$TOPLEVEL")/onp-worktrees/AlfaMatriz-permissoes-editar-e-imprimir"
...
mesclar_faixa() { # $1=faixa $2=branch $3=worktree $4=exit-da-faixa
  if [ "$4" -ne 0 ]; then
    ...
    amarelo "  reexecute só ela: bash .spec/features/permissoes-editar-e-imprimir/executar-tarefas.sh --faixa $1"
...
executar_faixa_1() {
  ...
  preparar_worktree 'faixa-1' 'spec/permissoes-editar-e-imprimir-faixa-1' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-1' --estado executando --tentativa "$(tentativa 'faixa-1')"
  : > "$LOG_DIR/faixa-1.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-1' 'T-001' 'Você executa UMA tarefa da feature "permissoes-editar-e-imprimir" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/permissoes-editar-e-imprimir/spec.md, .spec/features/permissoes-editar-e-imprimir/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-001 — "A ação `imprimir` passa a recusar"
  critérios/refs: AC-272 (Sem "imprimir", a exportação do faturamento é recusada), AC-273 (Sem "imprimir", baixar boleto e nota fiscal é recusado), AC-274 (Quem tem "imprimir" continua exportando e baixando)
  arquivos permitidos (e seus testes): routes/web.php, app/Http/Controllers/FaturamentoController.php, tests/Feature/Permissoes/ImprimirTest.php
  mensagem de commit: "T-001 permissoes-editar-e-imprimir: A ação `imprimir` passa a recusar"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium &&
    rodar_tarefa 'faixa-1' 'T-004' 'Você executa UMA tarefa da feature "permissoes-editar-e-imprimir" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/permissoes-editar-e-imprimir/spec.md, .spec/features/permissoes-editar-e-imprimir/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-004 — "As edições que usam POST"
  critérios/refs: AC-278 (As edições que usam POST também exigem "editar")
  arquivos permitidos (e seus testes): routes/web.php, tests/Feature/Permissoes/EdicaoPorPostTest.php
  mensagem de commit: "T-004 permissoes-editar-e-imprimir: As edições que usam POST"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-1' 'spec/permissoes-editar-e-imprimir-faixa-1' "$WT" "$st" || return 1
  marcar_concluidas T-001 T-004
  return 0
}
...
executar_faixa_2() {
  ...
  preparar_worktree 'faixa-2' 'spec/permissoes-editar-e-imprimir-faixa-2' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-2' --estado executando --tentativa "$(tentativa 'faixa-2')"
  : > "$LOG_DIR/faixa-2.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-2' 'T-002' 'Você executa UMA tarefa da feature "permissoes-editar-e-imprimir" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/permissoes-editar-e-imprimir/spec.md, .spec/features/permissoes-editar-e-imprimir/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-002 — "A coluna `editar` e o backfill"
  critérios/refs: AC-279 (Ninguém perde a edição na publicação), AC-280 (O Administrador continua podendo tudo)
  arquivos permitidos (e seus testes): database/migrations/2026_08_15_090000_separar_editar_de_incluir.php, database/seeders/PerfilPermissaoSeeder.php, tests/Feature/Permissoes/MigracaoEditarTest.php
  mensagem de commit: "T-002 permissoes-editar-e-imprimir: A coluna `editar` e o backfill"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-2' 'spec/permissoes-editar-e-imprimir-faixa-2' "$WT" "$st" || return 1
...
executar_faixa_3() {
  ...
  preparar_worktree 'faixa-3' 'spec/permissoes-editar-e-imprimir-faixa-3' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-3' --estado executando --tentativa "$(tentativa 'faixa-3')"
  : > "$LOG_DIR/faixa-3.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-3' 'T-003' 'Você executa UMA tarefa da feature "permissoes-editar-e-imprimir" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/permissoes-editar-e-imprimir/spec.md, .spec/features/permissoes-editar-e-imprimir/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-003 — "`editar` vira ação de verdade no domínio"
  critérios/refs: AC-275 (A grade de perfis passa a ter cinco caixas), AC-276 (Sem "editar", alterar um registro existente é recusado), AC-277 (Sem "editar", cadastrar continua funcionando)
  arquivos permitidos (e seus testes): app/Models/User.php, app/Http/Middleware/ChecarPermissao.php, app/Http/Controllers/PerfilController.php, resources/views/usuarios/_permissoes.blade.php, tests/Feature/Permissoes/GradeComEditarTest.php
  mensagem de commit: "T-003 permissoes-editar-e-imprimir: `editar` vira ação de verdade no domínio"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-3' 'spec/permissoes-editar-e-imprimir-faixa-3' "$WT" "$st" || return 1
...
encerrar() { # $1=escopo
  ...
  if [ "$COM_GATE" -eq 0 ]; then
    ...
    if [ -z "$FALHAS" ]; then
      ...
      amarelo "  para o veredito: bash .spec/features/permissoes-editar-e-imprimir/executar-tarefas.sh --gate"
      exit 0
    fi
...
listar() {
  ...
  echo "  faixa-1  onda 1  T-001, T-004"
