#!/usr/bin/env bash
# executar-tarefas.sh — gerado por `onp-spec plano quem-revisa-e-quem-testa` em 2026-08-15 16:42
...
#
# resumo do que está rolando, a qualquer momento: onp-spec resumo quem-revisa-e-quem-testa
set -u
set -o pipefail

RUN_ID='AlfaMatriz-quem-revisa-e-quem-testa-msultu9h'
FEATURE='quem-revisa-e-quem-testa'
BASE_BRANCH='spec/quem-revisa-e-quem-testa'
...
preparar_ambiente() {
  ...
  git ls-files --error-unmatch -- '.spec/features/quem-revisa-e-quem-testa/spec.md' >/dev/null 2>&1 || falhar "spec.md não está commitada — os worktrees das faixas precisam dela no git"
  ...
  git worktree prune
  LOG_DIR="$(dirname "$TOPLEVEL")/onp-worktrees/AlfaMatriz-quem-revisa-e-quem-testa-logs"
  WT_BASE="$(dirname "$TOPLEVEL")/onp-worktrees/AlfaMatriz-quem-revisa-e-quem-testa"
...
mesclar_faixa() { # $1=faixa $2=branch $3=worktree $4=exit-da-faixa
  if [ "$4" -ne 0 ]; then
    ...
    amarelo "  reexecute só ela: bash .spec/features/quem-revisa-e-quem-testa/executar-tarefas.sh --faixa $1"
...
# ── sequencial T-127 (ordem do tasks.md) ──
executar_seq_T_127() {
  info 'sequencial T-127 — O motor: apontar ao mover, recomeçar ao entrar'
  if rodar_tarefa seq 'T-127' 'Você executa UMA tarefa da feature "quem-revisa-e-quem-testa" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/quem-revisa-e-quem-testa/spec.md, .spec/features/quem-revisa-e-quem-testa/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-127 — "O motor: apontar ao mover, recomeçar ao entrar"
  critérios/refs: AC-316 (O apontado vira o outro lado e fica sabendo), AC-317 (A pergunta seguinte já aponta para a pessoa certa), AC-318 (Entrar num portão sem apontar recomeça a conversa), AC-319 (O responsável não pode ser o apontado)
  arquivos permitidos (e seus testes): app/Services/FluxoTarefaService.php, app/Http/Controllers/TarefaController.php, tests/Feature/TarefasDesenvolvimento/QuemRevisaEQuemTestaTest.php
  mensagem de commit: "T-127 quem-revisa-e-quem-testa: O motor: apontar ao mover, recomeçar ao entrar"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium >> "$LOG_DIR/seq.log" 2>&1; then
    ...
    if [ -n "$(git status --porcelain)" ]; then
      git add -A && git commit -q -m 'T-127 quem-revisa-e-quem-testa: O motor: apontar ao mover, recomeçar ao entrar (auto-commit do plano)'
    fi
    marcar_concluidas T-127
    verde "✔ T-127 concluída"
    return 0
  fi
  vermelho "✘ T-127 falhou (log: $LOG_DIR/seq.log)"
  amarelo "  reexecute só ela: bash .spec/features/quem-revisa-e-quem-testa/executar-tarefas.sh --seq T-127"
  FALHAS="$FALHAS T-127"
  return 1
}

# ── sequencial T-128 (ordem do tasks.md) ──
executar_seq_T_128() {
  info 'sequencial T-128 — O seletor no painel do movimento'
  if rodar_tarefa seq 'T-128' 'Você executa UMA tarefa da feature "quem-revisa-e-quem-testa" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/quem-revisa-e-quem-testa/spec.md, .spec/features/quem-revisa-e-quem-testa/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-128 — "O seletor no painel do movimento"
  critérios/refs: AC-315 (O painel do movimento oferece a escolha nas entradas dos portões)
  arquivos permitidos (e seus testes): resources/views/tarefas/_painel-motivo.blade.php, resources/views/tarefas/index.blade.php, tests/Feature/TarefasDesenvolvimento/QuemRevisaEQuemTestaTest.php
  mensagem de commit: "T-128 quem-revisa-e-quem-testa: O seletor no painel do movimento"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium >> "$LOG_DIR/seq.log" 2>&1; then
    # commit de segurança se o agente esqueceu (rastreabilidade > perfeição)
    if [ -n "$(git status --porcelain)" ]; then
      git add -A && git commit -q -m 'T-128 quem-revisa-e-quem-testa: O seletor no painel do movimento (auto-commit do plano)'
    fi
    marcar_concluidas T-128
    verde "✔ T-128 concluída"
    return 0
  fi
  vermelho "✘ T-128 falhou (log: $LOG_DIR/seq.log)"
  amarelo "  reexecute só ela: bash .spec/features/quem-revisa-e-quem-testa/executar-tarefas.sh --seq T-128"
  FALHAS="$FALHAS T-128"
  return 1
}
...
executar_seq_T_129() {
  info 'sequencial T-129 — A tarja do teste nomeia o testador'
  if rodar_tarefa seq 'T-129' 'Você executa UMA tarefa da feature "quem-revisa-e-quem-testa" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/quem-revisa-e-quem-testa/spec.md, .spec/features/quem-revisa-e-quem-testa/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-129 — "A tarja do teste nomeia o testador"
  critérios/refs: AC-320 (A tarja do teste diz de quem se espera o teste)
  arquivos permitidos (e seus testes): resources/views/tarefas/_avisos-da-tarefa.blade.php, tests/Feature/TarefasDesenvolvimento/QuemRevisaEQuemTestaTest.php
  mensagem de commit: "T-129 quem-revisa-e-quem-testa: A tarja do teste nomeia o testador"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium >> "$LOG_DIR/seq.log" 2>&1; then
    # commit de segurança se o agente esqueceu (rastreabilidade > perfeição)
    if [ -n "$(git status --porcelain)" ]; then
      git add -A && git commit -q -m 'T-129 quem-revisa-e-quem-testa: A tarja do teste nomeia o testador (auto-commit do plano)'
    fi
  ...
  amarelo "  reexecute só ela: bash .spec/features/quem-revisa-e-quem-testa/executar-tarefas.sh --seq T-129"
  FALHAS="$FALHAS T-129"
  return 1
}
...
encerrar() { # $1=escopo
  ...
  if [ "$COM_GATE" -eq 0 ]; then
    ...
    if [ -z "$FALHAS" ]; then
      ...
      amarelo "  para o veredito: bash .spec/features/quem-revisa-e-quem-testa/executar-tarefas.sh --gate"
      exit 0
    fi
...
executar_tudo() {
  ...
  executar_seq_T_127 || true
  executar_seq_T_128 || true
...
listar() {
  ...
  echo "  seq       T-127 (sequencial)"
  echo "  seq       T-128 (sequencial)"
...
case "$MODO" in
  ...
  seq)
    case "$ALVO" in
      T-127) evento --tipo inicio --escopo "seq:T-127"; iniciar_resumos; executar_seq_T_127 || true; encerrar "seq:T-127" ;;
      T-128) evento --tipo inicio --escopo "seq:T-128"; iniciar_resumos; executar_seq_T_128 || true; encerrar "seq:T-128" ;;
