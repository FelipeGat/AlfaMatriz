#!/usr/bin/env bash
# executar-tarefas.sh — gerado por `onp-spec plano desempenho-do-sistema` em 2026-08-14 14:12
...
#
# resumo do que está rolando, a qualquer momento: onp-spec resumo desempenho-do-sistema
set -u
set -o pipefail

RUN_ID='AlfaMatriz-desempenho-do-sistema-mst110mr'
FEATURE='desempenho-do-sistema'
BASE_BRANCH='spec/desempenho-do-sistema'
...
preparar_ambiente() {
  ...
  git ls-files --error-unmatch -- '.spec/features/desempenho-do-sistema/spec.md' >/dev/null 2>&1 || falhar "spec.md não está commitada — os worktrees das faixas precisam dela no git"
  ...
  git worktree prune
  LOG_DIR="$(dirname "$TOPLEVEL")/onp-worktrees/AlfaMatriz-desempenho-do-sistema-logs"
  WT_BASE="$(dirname "$TOPLEVEL")/onp-worktrees/AlfaMatriz-desempenho-do-sistema"
...
mesclar_faixa() { # $1=faixa $2=branch $3=worktree $4=exit-da-faixa
  if [ "$4" -ne 0 ]; then
    ...
    amarelo "  reexecute só ela: bash .spec/features/desempenho-do-sistema/executar-tarefas.sh --faixa $1"
...
executar_faixa_1() {
  ...
  preparar_worktree 'faixa-1' 'spec/desempenho-do-sistema-faixa-1' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-1' --estado executando --tentativa "$(tentativa 'faixa-1')"
  : > "$LOG_DIR/faixa-1.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-1' 'T-103' 'Você executa UMA tarefa da feature "desempenho-do-sistema" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/desempenho-do-sistema/spec.md, .spec/features/desempenho-do-sistema/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-103 — "Permissão resolvida uma vez por requisição"
  critérios/refs: AC-236 (O quadro não repergunta a permissão a cada card), AC-237 (O custo da tela não cresce com o volume de tarefas), AC-238 (A sidebar decide os 14 links sem 14 consultas), AC-239 (Quem não tem permissão continua sem ver e sem entrar)
  arquivos permitidos (e seus testes): app/Models/User.php, tests/Feature/Desempenho/PermissaoEmCacheTest.php
  mensagem de commit: "T-103 desempenho-do-sistema: Permissão resolvida uma vez por requisição"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-1' 'spec/desempenho-do-sistema-faixa-1' "$WT" "$st" || return 1
...
executar_faixa_2() {
  ...
  preparar_worktree 'faixa-2' 'spec/desempenho-do-sistema-faixa-2' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-2' --estado executando --tentativa "$(tentativa 'faixa-2')"
  : > "$LOG_DIR/faixa-2.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-2' 'T-104' 'Você executa UMA tarefa da feature "desempenho-do-sistema" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/desempenho-do-sistema/spec.md, .spec/features/desempenho-do-sistema/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-104 — "O sino consulta uma vez por requisição"
  critérios/refs: AC-243 (Uma leitura de notificações por requisição)
  arquivos permitidos (e seus testes): app/Providers/AppServiceProvider.php, tests/Feature/Desempenho/SinoUmaVezTest.php
  mensagem de commit: "T-104 desempenho-do-sistema: O sino consulta uma vez por requisição"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' low
  ...
  local st=$?
  mesclar_faixa 'faixa-2' 'spec/desempenho-do-sistema-faixa-2' "$WT" "$st" || return 1
...
executar_faixa_3() {
  ...
  preparar_worktree 'faixa-3' 'spec/desempenho-do-sistema-faixa-3' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-3' --estado executando --tentativa "$(tentativa 'faixa-3')"
  : > "$LOG_DIR/faixa-3.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-3' 'T-105' 'Você executa UMA tarefa da feature "desempenho-do-sistema" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/desempenho-do-sistema/spec.md, .spec/features/desempenho-do-sistema/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-105 — "O quadro para de imprimir um modal por card"
  critérios/refs: AC-240 (O HTML do quadro para de crescer com o volume), AC-241 (Clicar no card abre a tarefa inteira), AC-242 (As ações do modal continuam funcionando)
  arquivos permitidos (e seus testes): resources/views/tarefas/index.blade.php, resources/views/tarefas/_modais.blade.php, app/Http/Controllers/TarefaController.php, routes/web.php, tests/Feature/Desempenho/QuadroLeveTest.php
  mensagem de commit: "T-105 desempenho-do-sistema: O quadro para de imprimir um modal por card"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' high
  ...
  local st=$?
  mesclar_faixa 'faixa-3' 'spec/desempenho-do-sistema-faixa-3' "$WT" "$st" || return 1
...
executar_faixa_4() {
  ...
  preparar_worktree 'faixa-4' 'spec/desempenho-do-sistema-faixa-4' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-4' --estado executando --tentativa "$(tentativa 'faixa-4')"
  : > "$LOG_DIR/faixa-4.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-4' 'T-106' 'Você executa UMA tarefa da feature "desempenho-do-sistema" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/desempenho-do-sistema/spec.md, .spec/features/desempenho-do-sistema/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-106 — "Painéis: as séries saem de uma consulta agrupada"
  critérios/refs: AC-244 (O Painel Financeiro cabe em 30 consultas), AC-245 (O Centro de Controle cabe em 30 consultas), AC-246 (Os números continuam exatamente os mesmos)
  arquivos permitidos (e seus testes): app/Services/IndicadoresService.php, app/Http/Controllers/PainelController.php, tests/Feature/Desempenho/PaineisTest.php
  mensagem de commit: "T-106 desempenho-do-sistema: Painéis: as séries saem de uma consulta agrupada"
...
- NÃO edite tasks.md, NÃO rode onp-spec verify/audit e NÃO toque em outras tarefas — o orquestrador cuida disso.
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' high &&
    rodar_tarefa 'faixa-4' 'T-107' 'Você executa UMA tarefa da feature "desempenho-do-sistema" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/desempenho-do-sistema/spec.md, .spec/features/desempenho-do-sistema/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-107 — "Ranking de sistemas sem uma consulta por sistema"
  critérios/refs: AC-247 (O ranking de sistemas não consulta por sistema)
  arquivos permitidos (e seus testes): app/Models/Sistema.php, app/Services/IndicadoresService.php, tests/Feature/Desempenho/RankingSistemasTest.php
  mensagem de commit: "T-107 desempenho-do-sistema: Ranking de sistemas sem uma consulta por sistema"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium
  ...
  local st=$?
  mesclar_faixa 'faixa-4' 'spec/desempenho-do-sistema-faixa-4' "$WT" "$st" || return 1
  marcar_concluidas T-106 T-107
  return 0
}
...
executar_faixa_5() {
  ...
  preparar_worktree 'faixa-5' 'spec/desempenho-do-sistema-faixa-5' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-5' --estado executando --tentativa "$(tentativa 'faixa-5')"
  : > "$LOG_DIR/faixa-5.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-5' 'T-108' 'Você executa UMA tarefa da feature "desempenho-do-sistema" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/desempenho-do-sistema/spec.md, .spec/features/desempenho-do-sistema/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-108 — "A previsão de faturamento consulta em bloco"
  critérios/refs: AC-248 (O custo da previsão não cresce com o número de revendas), AC-249 (O valor previsto não muda)
  arquivos permitidos (e seus testes): app/Services/FaturamentoService.php, tests/Feature/Desempenho/PrevisaoEmBlocoTest.php
  mensagem de commit: "T-108 desempenho-do-sistema: A previsão de faturamento consulta em bloco"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' high
  ...
  local st=$?
  mesclar_faixa 'faixa-5' 'spec/desempenho-do-sistema-faixa-5' "$WT" "$st" || return 1
...
encerrar() { # $1=escopo
  ...
  if [ "$COM_GATE" -eq 0 ]; then
    ...
    if [ -z "$FALHAS" ]; then
      ...
      amarelo "  para o veredito: bash .spec/features/desempenho-do-sistema/executar-tarefas.sh --gate"
      exit 0
    fi
...
listar() {
  ...
  echo "  faixa-4  onda 2  T-106, T-107"
  echo "  faixa-5  onda 2  T-108"
  echo
