#!/usr/bin/env bash
# executar-tarefas.sh — gerado por `onp-spec plano clientes-via-revenda` em 2026-08-08 13:57
...
#
# resumo do que está rolando, a qualquer momento: onp-spec resumo clientes-via-revenda
set -u
set -o pipefail

RUN_ID='AlfaMatriz-clientes-via-revenda-mskfueum'
FEATURE='clientes-via-revenda'
BASE_BRANCH='spec/clientes-via-revenda'
...
preparar_ambiente() {
  ...
  git ls-files --error-unmatch -- '.spec/features/clientes-via-revenda/spec.md' >/dev/null 2>&1 || falhar "spec.md não está commitada — os worktrees das faixas precisam dela no git"
  ...
  git worktree prune
  LOG_DIR="$(dirname "$TOPLEVEL")/onp-worktrees/AlfaMatriz-clientes-via-revenda-logs"
  WT_BASE="$(dirname "$TOPLEVEL")/onp-worktrees/AlfaMatriz-clientes-via-revenda"
...
mesclar_faixa() { # $1=faixa $2=branch $3=worktree $4=exit-da-faixa
  if [ "$4" -ne 0 ]; then
    ...
    amarelo "  reexecute só ela: bash .spec/features/clientes-via-revenda/executar-tarefas.sh --faixa $1"
...
# ── faixa-1: T-053 T-054 T-056 T-057 ──
executar_faixa_1() {
  local WT="$WT_BASE-faixa-1"
  preparar_worktree 'faixa-1' 'spec/clientes-via-revenda-faixa-1' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-1' --estado executando --tentativa "$(tentativa 'faixa-1')"
  : > "$LOG_DIR/faixa-1.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-1' 'T-053' 'Você executa UMA tarefa da feature "clientes-via-revenda" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/clientes-via-revenda/spec.md, .spec/features/clientes-via-revenda/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-053 — "Revenda obrigatória no cadastro de cliente"
  critérios/refs: AC-072 (Cadastro de cliente exige revenda)
  arquivos permitidos (e seus testes): app/Http/Controllers/ClienteController.php, resources/views/clientes/_form.blade.php, tests/Feature/Redesign/ClientesTest.php
  mensagem de commit: "T-053 clientes-via-revenda: Revenda obrigatória no cadastro de cliente"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' low &&
    rodar_tarefa 'faixa-1' 'T-054' 'Você executa UMA tarefa da feature "clientes-via-revenda" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/clientes-via-revenda/spec.md, .spec/features/clientes-via-revenda/tasks.md e .spec/constituicao.md.
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium &&
    rodar_tarefa 'faixa-1' 'T-056' 'Você executa UMA tarefa da feature "clientes-via-revenda" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/clientes-via-revenda/spec.md, .spec/features/clientes-via-revenda/tasks.md e .spec/constituicao.md.
...
- NÃO edite tasks.md, NÃO rode onp-spec verify/audit e NÃO toque em outras tarefas — o orquestrador cuida disso.
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium &&
    rodar_tarefa 'faixa-1' 'T-057' 'Você executa UMA tarefa da feature "clientes-via-revenda" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/clientes-via-revenda/spec.md, .spec/features/clientes-via-revenda/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-057 — "Admin libera a licença do cliente pelo AlfaGym"
  critérios/refs: AC-078 (Liberar licença envia tipo, valor e observação ao AlfaGym), AC-079 (Liberar licença não conta como avulso), AC-080 (Recusa do AlfaGym aparece como erro sem gravar nada)
  arquivos permitidos (e seus testes): app/Http/Controllers/ClienteController.php, app/Services/LiberadorLicencaAlfaGymService.php, resources/views/clientes/index.blade.php, routes/web.php, tests/Feature/SincronizadorAlfaGymTest.php, tests/Feature/Redesign/ClientesTest.php
  mensagem de commit: "T-057 clientes-via-revenda: Admin libera a licença do cliente pelo AlfaGym"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' high
  ...
  local st=$?
  mesclar_faixa 'faixa-1' 'spec/clientes-via-revenda-faixa-1' "$WT" "$st" || return 1
  marcar_concluidas T-053 T-054 T-056 T-057
  return 0
}
...
executar_faixa_2() {
  ...
  preparar_worktree 'faixa-2' 'spec/clientes-via-revenda-faixa-2' "$WT" || return 1
  evento --tipo faixa --faixa 'faixa-2' --estado executando --tentativa "$(tentativa 'faixa-2')"
  : > "$LOG_DIR/faixa-2.log"
  (
    cd "$WT" || exit 9
    rodar_tarefa 'faixa-2' 'T-055' 'Você executa UMA tarefa da feature "clientes-via-revenda" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/clientes-via-revenda/spec.md, .spec/features/clientes-via-revenda/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-055 — "Aba "Clientes" dentro da tela de Revendas"
  critérios/refs: AC-075 (A tela de Revendas mostra a aba "Clientes"), AC-076 (Usuário de revenda vê só a própria revenda na aba)
  arquivos permitidos (e seus testes): app/Http/Controllers/RevendaController.php, resources/views/revendas/index.blade.php, resources/views/layouts/navigation.blade.php, tests/Feature/Redesign/RevendasTest.php, tests/Feature/Autorizacao/EscopoDeRevendaTest.php
  mensagem de commit: "T-055 clientes-via-revenda: Aba "Clientes" dentro da tela de Revendas"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' high
  ...
  local st=$?
  mesclar_faixa 'faixa-2' 'spec/clientes-via-revenda-faixa-2' "$WT" "$st" || return 1
...
encerrar() { # $1=escopo
  ...
  if [ "$COM_GATE" -eq 0 ]; then
    ...
    if [ -z "$FALHAS" ]; then
      ...
      amarelo "  para o veredito: bash .spec/features/clientes-via-revenda/executar-tarefas.sh --gate"
      exit 0
    fi
...
listar() {
  ...
  echo "  faixa-1  onda 1  T-053, T-054, T-056, T-057"
  echo "  faixa-2  onda 1  T-055"
  echo
