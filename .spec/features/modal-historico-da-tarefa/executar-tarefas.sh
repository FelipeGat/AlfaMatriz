#!/usr/bin/env bash
# executar-tarefas.sh — gerado por `onp-spec plano modal-historico-da-tarefa` em 2026-08-15 10:16
...
#
# resumo do que está rolando, a qualquer momento: onp-spec resumo modal-historico-da-tarefa
set -u
set -o pipefail

RUN_ID='AlfaMatriz-modal-historico-da-tarefa-msu81qg1'
FEATURE='modal-historico-da-tarefa'
BASE_BRANCH='spec/modal-historico-da-tarefa'
...
preparar_ambiente() {
  ...
  git ls-files --error-unmatch -- '.spec/features/modal-historico-da-tarefa/spec.md' >/dev/null 2>&1 || falhar "spec.md não está commitada — os worktrees das faixas precisam dela no git"
  ...
  git worktree prune
  LOG_DIR="$(dirname "$TOPLEVEL")/onp-worktrees/AlfaMatriz-modal-historico-da-tarefa-logs"
  WT_BASE="$(dirname "$TOPLEVEL")/onp-worktrees/AlfaMatriz-modal-historico-da-tarefa"
...
mesclar_faixa() { # $1=faixa $2=branch $3=worktree $4=exit-da-faixa
  if [ "$4" -ne 0 ]; then
    ...
    amarelo "  reexecute só ela: bash .spec/features/modal-historico-da-tarefa/executar-tarefas.sh --faixa $1"
...
# ── sequencial T-118 (ordem do tasks.md) ──
executar_seq_T_118() {
  info 'sequencial T-118 — Autor do evento: banco, serviço e modelo'
  if rodar_tarefa seq 'T-118' 'Você executa UMA tarefa da feature "modal-historico-da-tarefa" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/modal-historico-da-tarefa/spec.md, .spec/features/modal-historico-da-tarefa/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-118 — "Autor do evento: banco, serviço e modelo"
  critérios/refs: AC-301 (Movimento novo grava quem o fez)
  arquivos permitidos (e seus testes): database/migrations/2026_08_15_100000_autor_do_evento_da_tarefa.php, app/Services/FluxoTarefaService.php, app/Models/TarefaEvento.php, tests/Feature/TarefasDesenvolvimento/AutorDoEventoTest.php
  mensagem de commit: "T-118 modal-historico-da-tarefa: Autor do evento: banco, serviço e modelo"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium >> "$LOG_DIR/seq.log" 2>&1; then
    ...
    if [ -n "$(git status --porcelain)" ]; then
      git add -A && git commit -q -m 'T-118 modal-historico-da-tarefa: Autor do evento: banco, serviço e modelo (auto-commit do plano)'
    fi
    marcar_concluidas T-118
    verde "✔ T-118 concluída"
    return 0
  fi
  vermelho "✘ T-118 falhou (log: $LOG_DIR/seq.log)"
  amarelo "  reexecute só ela: bash .spec/features/modal-historico-da-tarefa/executar-tarefas.sh --seq T-118"
  FALHAS="$FALHAS T-118"
  return 1
}

# ── sequencial T-119 (ordem do tasks.md) ──
executar_seq_T_119() {
  info 'sequencial T-119 — Partial da linha do tempo da tarefa'
  if rodar_tarefa seq 'T-119' 'Você executa UMA tarefa da feature "modal-historico-da-tarefa" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/modal-historico-da-tarefa/spec.md, .spec/features/modal-historico-da-tarefa/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-119 — "Partial da linha do tempo da tarefa"
  critérios/refs: AC-295 (A linha do tempo mostra cada movimento da tarefa), AC-296 (Etapas aposentadas aparecem com o nome, nunca a chave crua), AC-302 (A linha do tempo mostra o autor quando ele existe)
  arquivos permitidos (e seus testes): resources/views/tarefas/_linha-do-tempo.blade.php
  mensagem de commit: "T-119 modal-historico-da-tarefa: Partial da linha do tempo da tarefa"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' medium >> "$LOG_DIR/seq.log" 2>&1; then
    # commit de segurança se o agente esqueceu (rastreabilidade > perfeição)
    if [ -n "$(git status --porcelain)" ]; then
      git add -A && git commit -q -m 'T-119 modal-historico-da-tarefa: Partial da linha do tempo da tarefa (auto-commit do plano)'
    fi
    marcar_concluidas T-119
    verde "✔ T-119 concluída"
    return 0
  fi
  vermelho "✘ T-119 falhou (log: $LOG_DIR/seq.log)"
  amarelo "  reexecute só ela: bash .spec/features/modal-historico-da-tarefa/executar-tarefas.sh --seq T-119"
  FALHAS="$FALHAS T-119"
  return 1
}

# ── sequencial T-120 (ordem do tasks.md) ──
executar_seq_T_120() {
  info 'sequencial T-120 — Página do histórico: linha clicável e modal completo'
  if rodar_tarefa seq 'T-120' 'Você executa UMA tarefa da feature "modal-historico-da-tarefa" (fluxo onp-spec, spec-anchored).
Leia primeiro: .spec/features/modal-historico-da-tarefa/spec.md, .spec/features/modal-historico-da-tarefa/tasks.md e .spec/constituicao.md.

Sua tarefa (somente ela):
T-120 — "Página do histórico: linha clicável e modal completo"
  critérios/refs: AC-293 (A linha da tabela abre o modal de histórico completo), AC-294 (Os controles da linha continuam agindo sem abrir o modal), AC-297 (A conversa e os anexos vivem dentro do mesmo modal), AC-298 (O checklist aparece com o estado final dos itens), AC-299 (Os relatórios de teste aparecem com veredito e notas), AC-300 (Seção sem conteúdo não aparece)
  arquivos permitidos (e seus testes): resources/views/tarefas/historico.blade.php, app/Http/Controllers/TarefaController.php, tests/Feature/TarefasDesenvolvimento/ModalHistoricoDaTarefaTest.php
  mensagem de commit: "T-120 modal-historico-da-tarefa: Página do histórico: linha clicável e modal completo"
...
- Ao final de CADA tarefa: `git add` só no que você tocou e um commit próprio.' 'claude-sonnet-5' high >> "$LOG_DIR/seq.log" 2>&1; then
    # commit de segurança se o agente esqueceu (rastreabilidade > perfeição)
    if [ -n "$(git status --porcelain)" ]; then
      git add -A && git commit -q -m 'T-120 modal-historico-da-tarefa: Página do histórico: linha clicável e modal completo (auto-commit do plano)'
    fi
    marcar_concluidas T-120
    verde "✔ T-120 concluída"
    return 0
  fi
  vermelho "✘ T-120 falhou (log: $LOG_DIR/seq.log)"
  amarelo "  reexecute só ela: bash .spec/features/modal-historico-da-tarefa/executar-tarefas.sh --seq T-120"
  FALHAS="$FALHAS T-120"
  return 1
}
...
encerrar() { # $1=escopo
  ...
  if [ "$COM_GATE" -eq 0 ]; then
    ...
    if [ -z "$FALHAS" ]; then
      ...
      amarelo "  para o veredito: bash .spec/features/modal-historico-da-tarefa/executar-tarefas.sh --gate"
      exit 0
    fi
...
executar_tudo() {
  ...
  executar_seq_T_118 || true
  executar_seq_T_119 || true
  executar_seq_T_120 || true
  encerrar tudo
}

listar() {
  echo "execução: $RUN_ID (feature $FEATURE, branch $BASE_BRANCH)"
  echo "  seq       T-118 (sequencial)"
  echo "  seq       T-119 (sequencial)"
  echo "  seq       T-120 (sequencial)"
  echo
...
case "$MODO" in
  ...
  seq)
    case "$ALVO" in
      T-118) evento --tipo inicio --escopo "seq:T-118"; iniciar_resumos; executar_seq_T_118 || true; encerrar "seq:T-118" ;;
      T-119) evento --tipo inicio --escopo "seq:T-119"; iniciar_resumos; executar_seq_T_119 || true; encerrar "seq:T-119" ;;
      T-120) evento --tipo inicio --escopo "seq:T-120"; iniciar_resumos; executar_seq_T_120 || true; encerrar "seq:T-120" ;;
